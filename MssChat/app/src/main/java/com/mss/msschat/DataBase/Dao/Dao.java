...
public abstract class Dao<T> {
     ...
     */
    public void insertOrReplace(T obj, String field, String value) {
        Dto dto = (Dto) obj;
        // if (value != "-1") {
        if (exists(field, value)) {
            replaceDto(dto, " WHERE " + field + " = '" + value + "'");
        } else {
            insertDto(dto);
        }
        // } else {
     ...
     */
    public void insertOrReplace(JSONObject json, String field, String value) throws JSONException {
        T obj = buildDto(json);
        if (obj != null) {
            insertOrReplace(obj, field, value);
        } else {
    ...
    protected void replaceDto(Dto dto, String where) {
        List<String> columns = new ArrayList<String>();
        List<Object> values = new ArrayList<Object>();
        try {
            dto.prepareDbFields(columns, values);
